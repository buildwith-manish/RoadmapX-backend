...
// Also allows *.pages.dev preview URLs generated by Cloudflare for branch deploys.
app.use(cors({
  origin: function (origin, callback) {
    ...
    const isCfPreview = origin && /^https:\/\/[a-z0-9-]+\.roadmapx(-frontend)?\.pages\.dev$/.test(origin);
    if (!origin || allowed.includes(origin) || isCfPreview) {
      callback(null, true);
    } else {
...
//
//  GET  /auth/google/redirect  → builds the Google OAuth URL and redirects
//                                the browser to accounts.google.com.
//  GET  /auth/google/callback  → Google redirects here after the user
//                                consents; we verify the code, create/find
//                                the user, set the session, and redirect to
//                                the frontend.
//
//  Required env vars (same as One-Tap above):
//    GOOGLE_CLIENT_ID
//    GOOGLE_CLIENT_SECRET   ← NEW: needed for token exchange
//    FRONTEND_URL           ← e.g. https://roadmapx.pages.dev
//    BACKEND_URL            ← e.g. https://roadmapx-backend-3qmc.onrender.com
//
//  Google Console → Authorized redirect URIs must include:
//    https://roadmapx-backend-3qmc.onrender.com/auth/google/callback
// ────────────────────────────────────────────────────────────────────────

app.get("/auth/google/redirect", (req, res) => {
  if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
    return res.redirect(
      (process.env.FRONTEND_URL || "https://roadmapx.pages.dev") +
      "/login.html?google=fail&msg=Google+Sign-In+not+configured"
    );
  }

  const redirectUri = (process.env.BACKEND_URL || "https://roadmapx-backend-3qmc.onrender.com") +
...
app.get("/auth/google/callback", async (req, res) => {
  const frontendUrl = process.env.FRONTEND_URL || "https://roadmapx.pages.dev";
  const failRedirect = (msg) =>
    res.redirect(frontendUrl + "/login.html?google=fail&msg=" + encodeURIComponent(msg));

  if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET) {
    return failRedirect("Google Sign-In not configured.");
  }

  const { code, error } = req.query;
  if (error || !code) {
    return failRedirect(error === "access_denied" ? "Sign-in was cancelled." : "Google sign-in failed.");
  }
  ...
  try {
    ...
    // Verify the ID token using google-auth-library (same as One-Tap flow)
    if (!gsiClient) {
      return failRedirect("Google Sign-In not configured on server.");
    }
    ...
    // Create or find user (same logic as One-Tap)
    let user = await User.findOne({ email: googleEmail });
    if (!user) {
      const safeUsername = googleName.replace(/[^a-zA-Z0-9_]/g, "_").slice(0, 20) +
                           "_" + Math.random().toString(36).slice(2, 6);
    ...
    // Redirect back to frontend with username in query param so login.html
    // can write localStorage before auth_guard kicks in.
    res.redirect(
      frontendUrl + "/login.html?google=ok&user=" + encodeURIComponent(user.username)
    );
  } catch (err) {
